...
for item in myroot.findall('item'):
    ...
    pcs_on_stock = item.find('stock_quantity').text if item.find('stock_quantity')!=None else 0
    
    if product_list.get(product_id) != None:
        new_item = product_list.get(product_id) 
        new_item.append(str(pcs_on_stock))
